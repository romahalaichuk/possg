...
const Print = ({
    ...
    useEffect(() => {
        const savedPrintedItems = JSON.parse(localStorage.getItem("printedItems")) || [];
    ...
    const groupItemsByCategory = (items) => {
        ...
        Object.values(itemMap).forEach((item) => {
            ...
            } else if (item.category && item.category.toLowerCase().includes("wynos")) {
    ...
    const renderCategory = (category, items, hasPizza, hasMakaron) => (
        <div key={category} style={{ marginBottom: "5px" }}>
            ...
            {pickupTime && (
                <div className={customPickupTime.includes(":") ? "pickup-time-container" : ""}>
                    <p className={`pickup-time ${customPickupTime.includes(":") ? "pickup-time-na" : ""}`}>
            ...
            <>
                {Object.keys(groupedItems).map((category) =>
                    groupedItems[category].length > 0
                        ? renderCategory(category, groupedItems[category], hasPizza, hasMakaron)
                        : null
                )}
                {showDeliveryDetails &&
